...
test:
	...
	R --slave -e 'Sys.setenv(NOT_CRAN="true"); library(testthat); setwd(file.path(.libPaths()[1], "chromophobe", "tests")); system.time(test_check("chromophobe", filter="${file}", reporter=ifelse(nchar("${r}"), "${r}", "summary")))'
...
check: build
	-R CMD check --as-cran chromophobe_$(VERSION).tar.gz
	rm -rf chromophobe.Rcheck/
